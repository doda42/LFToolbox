% todo: check for "new in 0.4" "new in 0.3" etc
...
% todo: make ^ copy/paste correctly also
...
In formal presentation, indices $i,j,k,l$ are distinct from spatial coordinates $s,t,u,v$. In the toolbox, the latter is generally used and it is understood that the light fields are sampled and indexed.
...
\end{Verbatim}
To brighten the display through simple gamma correction, we can raise the light field to a power, but this first requires us to convert the light field to a floating point format.
\begin{Verbatim}
LF = LFConvertToFloat(LF);
LFDisp( LF.^0.5 )
axis image
\end{Verbatim}
This is not very efficient, as it raises the entire light field to the power 0.5, then displays only the central view. You can use the following trick to chain LFDisp commands:
\begin{Verbatim}
...
\end{Verbatim}
to display a crop in $u,v$. As with \CiteFunc{LFDisp}, \CiteFunc{LFDispTiles} returns the displayed image, allowing chaining:
\begin{Verbatim}
...
axis normal
\end{Verbatim}
this is best viewed with a non-square aspect ratio by resizing the window. The contrast on this image is low since all the differently sloped parts of the scene are adding together. We expect slices in $s,u$ to look like this:
\begin{Verbatim}
LFDisp(LF(9,:,226,:,:))
\end{Verbatim}
and so to get more of the structure apparent in the projection onto $s,u$, we can find the max along $s,v$ rather than the mean
\begin{Verbatim}
...
\item \textbf{Run \CiteFunc{LFReadGantryArray}} from the top-level of the Stanford samples:
\begin{Verbatim}
...
Some gantry imagery follows a different handedness in horizontal and vertical directions, effectively flipping the order of images in $s$ or $t$.  The demo function \CiteFunc{LFDemoBasicFiltGantry} contains a list of Stanford light fields and demonstrates how to flip the appropriate ones along $s$.
...
\end{Verbatim}
\item \textbf{Run \CiteFunc{LFFiltShiftSum}} to focus 
\begin{Verbatim}
...
\begin{figure}[tbh]
	...
	\caption{Linear super-resolution using \protect\CiteFunc{LFFiltShiftSum}. The inputs are small crops from larger light fields; `Focus' shows focus alone, mostly removing noise and limiting depth of field; '2D Interp' shows 2D cubic interpolation for comparison; and `Super-res' shows linear super-resolution with noticeably finer detail than focus alone or 2D interpolation. See \protect\CiteFunc{LFDemoRefocusSuperres} for usage.}
	\label{fig_RefocusSuperResolution}
\end{figure}

%These are tight crops on the Flowers sample ESLF and \SymbolText{IMG_6201.eslf.png} from the Flowers category of Stanford Multi-View Light Field dataset at \url{http://lightfields.stanford.edu}.
...
Each white image needs to be analyzed once in order to match a grid model to the lenslet locations. The utility \CiteFunc{LFUtilProcessWhiteImages} automatically builds a grid model for each white image in your white image folder. If you wish to store your white images in a structure other than the default, simply change the \SymbolText{WhiteImageDatabasePath} variable in \CiteFunc{LFUtilProcessWhiteImages} to point to your white images folder, or pass an argument \SymbolText{FileOptions} with the \SymbolText{WhiteImageDatabasePath} field set.
...
%-----------------------------
The \CiteFunc{LFUtilCalLensletCam} automatically progresses through the following calibration stages:
...
New in Version 0.3 is a helper function for building this matrix, \CiteFunc{LFCalDispRectIntrinsics}.  The recommended usage pattern is to load a light field, call \CiteFunc{LFCalDispRectIntrinsics} once to set up the default intrinsic matrix, manipulate the matrix, then visualize the manipulated sampling pattern prior to employing it in one or more rectification calls.  Assuming \SymbolText{IMG\_001} has been decoded but not rectified, a typical process might look like this:
\begin{Verbatim}
...
This version of the toolbox can read Lytro LFP files directly, using the function \CiteFunc{LFReadLFP}.  The toolbox is also compatible with \SymbolText{.raw} files extracted using an external LFP tool.
...
By default, \CiteFunc{LFUtilDecodeLytroFolder} recursively searches for files with any of these extensions, as well as the \SymbolText{raw} files employed by previous toolbox versions, and decodes anything it can make sense of.  Focal stacks and other files are also stored as .lfp files, and \CiteFunc{LFUtilDecodeLytroFolder} will ignore these files.


%----------------
\subsubsection{Thumbnails}

Thumbnails are built into some LFP files.  The function \CiteFunc{LFUtilExtractLFPThumbs} will find thumbnails and save them to disk.
...
First, reprocess white images as above, and re-decode calibration files, to benefit from the improved grid fit. Then, either use function parameters to force the calibration to redo previously complete steps, or delete generated files and re-run calibration. The relevant parameters to \CiteFunc{LFUtilCalLensletCam} are \SymbolText{CalOptions.ForceRedoCornerFinding} and \SymbolText{CalOptions.ForceRedoInit}. 

Once calibrations are re-processed, remove \SymbolText{CalibrationDatabase} then run \CiteFunc{LFUtilProcessCalibrations}.
