...
{\large Version 0.5.0 06-Apr-2020  xxxtodoxxx}
}
...
The toolbox is under active development. The most recent release and development versions are here: \url{https://github.com/doda42/LFToolbox}.

Sample light fields:
\begin{itemize}[leftmargin=4.2em,itemsep=0em,rightmargin=0.5cm]
\item LFToolbox v0.3 Sample Pack 1: \url{http://www-personal.acfr.usyd.edu.au/ddan1654/LFToolbox0.3_Samples1.zip}
...
\textbf{Light Field File Formats} The toolbox can load gantry / array-style folders of images, ESLF files, and raw lenslet-based images.
...
\textbf{Plenoptic 2.0} cameras are not well supported. Use with some cameras is possible but not well documented. Multi-focal lenslet-based cameras like the Raytrix are not well supported.

\textbf{Lytro Software} The original Lytro camera and the Lytro Illum are supported under Lytro Desktop 4 and 3. The toolbox will also load ESLF files generated by Lytro Power Tools.
...
todo: either via mathworks toolbox install, or from github

After unzipping the toolbox files to an appropriate location, run the convenience function \CiteFunction{LFMatlabPathSetup} to set up the Matlab path. This must be re-run every time Matlab restarts, so consider adding a line to \SymbolText{startup.m}, e.g.\ \begin{Verbatim}
...
Many toolbox functions accept optional parameters, taking on default values if no parameter is provided. This is done by omitting the parameter if it's at the end of a parameter list, or by passing an empty array \SymbolText{[]} within a parameter list.

todo: examples

See the help for \CiteFunction{LFUtilDecodeLytroFolder} for examples of this parameter-passing scheme.

Some functions accept structs of parameters. A helpful shorthand for calling these is using MATLAB's \SymbolText{struct} function, that allows inline struct construction. todo: examples.

%-----------------------------
\section{A Quick Tour}
\label{sect_QuickTour}
%-----------------------------
\subsection{Decoding the Sample Light Fields}
\label{sect_DecodingSamples}

\begin{enumerate}[leftmargin=1.5em,rightmargin=0.5cm]

\item \textbf{Download} the LF Toolbox and decompress into an appropriate location. Run \CiteFunction{LFMatlabPathSetup} to set up Matlab's paths.

\item \textbf{Download} the sample light field pack at \url{http://www-personal.acfr.usyd.edu.au/ddan1654/LFToolbox0.3_Samples1.zip} and decompress into its own folder. The samples folder structure was chosen for easy addition of your own cameras and calibrations:
...
From within Matlab \SymbolText{cd} into the top level of the samples folder.  If you are in the correct location, the Matlab command \SymbolText{ls} should list the top-level folders:
\begin{Verbatim}
Images
Cameras
\end{Verbatim}
...
\begin{figure}[tbh]
	...
	\caption{Example of a white image showing estimated lenslet centers as red dots.}
	\label{fig_WhiteImage}
\end{figure}

\item \textbf{Run \CiteFunction{LFUtilProcessWhiteImages}} to build a white image database. This searches the \SymbolText{Cameras} folder for white images, generating a lenslet grid model for each -- the grid models are saved as \SymbolText{*.grid.json}. The database of white images is saved as \SymbolText{Cameras/WhiteFileDatabase.mat}, and is used in selecting the appropriate white image for decoding each light field. 

\emph{As of version 0.3 of the toolbox, samples ship with precomputed \SymbolText{.grid.json} files. These files may be removed in order to force their re-generation.}  When doing so, for each lenslet grid model figures similar to Fig.~\ref{fig_WhiteImage} are presented for visual confirmation that the grid model is a good fit. Each figure shows a small subset of the whole frame to allow close inspection of the lenslets. Five such images are shown, one for each image corner and one for the central portion of the image. Each red dot should appear near the center of a lenslet -- i.e.\ near the brightest spot on each white bump, as depicted in Fig.~\ref{fig_WhiteImage}. It does not matter if there are a few rows of lenslets on the edges with no red markers.
...
\begin{figure}[tbh]
	...
	\caption{Decoded (top) and colour-corrected output (bottom) -- the white speckles in the bird image are due to a pane of grubby glass between the camera and the bird. Running \protect\CiteFunction{LFDispVidCirc} or \protect\CiteFunction{LFDispMousePan} creates a shifting-perspective view in which this is more clear.}
...
\item (optional) \textbf{Re-run \CiteFunction{LFUtilDecodeLytroFolder}} to perform colour correction. Use the commands
\begin{Verbatim}
...
As of version 0.3 of the toolbox, histogram adjustment is no longer automatically applied -- you may apply histogram equalization using \CiteFunction{LFHistEqualize}.  Illum imagery is not gamma-corrected.  Example colour-corrected output is shown in the bottom row of Fig.~\ref{fig_ExampleDecodeAndColourCorrectOutput}, and in Fig.~\ref{fig_ExampleIllumOutput}.

\item (optional) \textbf{Use \CiteFunction{LFDispVidCirc} or \CiteFunction{LFDispMousePan}} to visualize the light field with a shifting perspective. First load a light field using a command of the form
\begin{Verbatim}
load('Images/F01/IMG_0001__Decoded.mat');
\end{Verbatim}

to load the light field variable \SymbolText{LF}, then run either \SymbolText{LFDispVidCirc(LF)} or\\ \SymbolText{LFDispMousePan(LF)}. The former automatically animates a circular motion, while the latter allows mouse-controlled motion: click and drag in the window to change the perspective.  Try a larger display with \SymbolText{LFDispMousePan(LF, 2)} or \SymbolText{LFDispVidCirc(LF, [], [], 2)}, which doubles the displayed size. Close any open display windows before changing display sizes.
...
\label{sect_DownloadingStanfordSamples}
%-----------------------------

New in version 0.3 of the toolbox is \CiteFunction{LFReadGantryArray}, which will read gantry-style light fields such as those available at the Stanford Light Field Archive at \url{http://lightfield.stanford.edu}.  Gantry-style light fields are generally collected by a single camera mounted on a robotic gantry, though any light field stored as an ordered collection of individual images can be read using this function.
...
\item \textbf{Unzip} the archive into a dedicated folder for Stanford samples, following a structure like this:
...
\item \textbf{Run \CiteFunction{LFDispMousePan}} to display the loaded light field.
...
\begin{figure}
	...
	\caption{Examplex of filtering Lytro Illum imagery, showing the input (left) and shift-and-sum filter performing planar focus (right).}
...
Run one of \CiteFunction{LFDemoBasicFiltLytroF01} or \CiteFunction{LFDemoBasicFiltIllum} for a demo of some of the filters operating on Lytro imagery. This should be done from the top level of the Samples folder, after decoding the light fields as described in Sect.~\ref{sect_DecodingSamples}.  The best performance is obtained with rectified light fields.

Run \CiteFunction{LFDemoBasicFiltGantry} for a demo filtering the Stanford light fields.  This should be done from the top of the Stanford light fields folder, after downloading and unzipping the samples following the instructions in Sect.~\ref{sect_DownloadingStanfordSamples}.

Uncomment the appropriate line near the top of \CiteFunction{LFDemoBasicFiltGantry} to select from the 12 input light fields.
...
By default, \CiteFunction{LFUtilDecodeLytroFolder} recursively searches for files with any of these extensions, as well as the \SymbolText{raw} files employed by previous toolbox versions, and decodes anything it can make sense of.  Focal stacks and other files are also stored as .lfp files, and \CiteFunction{LFUtilDecodeLytroFolder} will ignore these files.


%----------------
\subsubsection{Thumbnails}

Thumbnails are built into some LFP files.  The function \CiteFunction{LFUtilExtractLFPThumbs} will find thumbnails and save them to disk.
...
%------------------------------------------------------------
\NewFunctionHeading{Decoding / Input}
%------------------------------------------------------------
\NewFunctionEntry{LFLytroDecodeImage}{}
{Decode a Lytro image from an \SymbolText{LFP} or \SymbolText{raw} file. Can be called directly to decode a single image into memory, or called indirectly through \CiteFunction{LFUtilDecodeLytroFolder}.}
...
%------------------------------------------------------------
\NewFunctionEntry{LFBuild2DFreqFan}{\NewTag}
{Construct a 2D fan filter in the frequency domain.  Apply this filter with \CiteFunction{LFFilt2DFFT}.}
\NewFunctionEntry{LFBuild2DFreqLine}{\NewTag}
{Construct a 2D line filter in the frequency domain.  The cascade of two line filters, applied in s,u and in t,v, is identical to a 4D planar filter, e.g.\ that constructed by \CiteFunction{LFBuild4DFreqPlane}.   Apply this filter with \CiteFunction{LFFilt2DFFT}.}
\NewFunctionEntry{LFBuild4DFreqDualFan}{\NewTag}
{Construct a 4D dual-fan filter in the frequency domain.    Apply this filter with \CiteFunction{LFFilt4DFFT}.}
\NewFunctionEntry{LFBuild4DFreqHypercone}{\NewTag}
{Construct a 4D hypercone filter in the frequency domain.   Apply this filter with \CiteFunction{LFFilt4DFFT}.}
\NewFunctionEntry{LFBuild4DFreqHyperfan}{\NewTag}
{Construct a 4D hyperfan filter in the frequency domain. This is useful for selecting objects over a range of depths from a lightfield, i.e.\ volumetric focus.  Apply this filter with \CiteFunction{LFFilt4DFFT}.}
\NewFunctionEntry{LFBuild4DFreqPlane}{\NewTag}
{Construct a 4D plane filter in the frequency domain.  This is useful for selecting objects at a single depth from a lightfield, and is similar in effect to refocus using, for example, the shift sum filter LFFiltShiftSum.  Apply this filter with \CiteFunction{LFFilt4DFFT}.}

\NewFunctionEntry{LFDemoBasicFiltLytroF01}{\NewTag}
{Demonstrates some of the basic filters on Lytro F01-captured imagery.}
\NewFunctionEntry{LFDemoBasicFiltIllum}{\NewTag}
{Demonstrates some of the basic filters on Lytro Illum-captured imagery.}
\NewFunctionEntry{LFDemoBasicFiltGantry}{\NewTag}
{Demonstrates some of the basic filters on Stanford light field archive light fields.}

\NewFunctionEntry{LFFilt2DFFT}{\NewTag}
{Applies a 2D frequency-domain filter to a 4D light field using the FFT.}
\NewFunctionEntry{LFFilt4DFFT}{\NewTag}
{Applies a 4D frequency-domain filter using the FFT.}
\NewFunctionEntry{LFFiltShiftSum}{\NewTag}
...
{Applies a colour balance vector, an RGB colour correction matrix, and gamma correction. Usage is demonstrated in \CiteFunction{LFUtilDecodeLytroFolder}.}

\NewFunctionEntry{LFHistEqualize}{}
{Adjusts the brightness of a light field based on histogram equalization. Capable of handling colour and monochrome images -- colour images are converted to HSV, and the value channel is equalized. Capable of handling different input dimensionalities including 2D images and 4D light fields. If a weight channel is present as a fourth colour channel, it is used to ignore zero-weight pixels. Usage is demonstrated in \CiteFunction{LFUtilDecodeLytroFolder}.}

%------------------------------------------------------------
\EndFunctionHeading
\NewFunctionHeading{Visualization}
%------------------------------------------------------------
\NewFunctionEntry{LFDisp}{\NewTag}
...
{Display 2D slices of the light field with a rudimentary parallax effect. Click and drag in the image to change the point of view. An optional parameter controls the display size. Note that darkening at the edges of unfiltered light fields mean that the effect is best near the center of the spatial range. For an automatically animated display, see \CiteFunction{LFDispVidCirc}. The function re-uses previously-opened light field display windows.  Note that changing display size requires that the display window be closed prior to calling this function.}

\NewFunctionEntry{LFDispVidCirc}{}
{Animated display showing 2D slices of the light field, similar to \CiteFunction{LFDispMousePan} except the motion is preset to a circular path. Optional parameters include the radius of the circular path, animation speed, and display size.}
...
{Visualize camera pose estimates. Called by \CiteFunction{LFUtilCalLensletCam}.}

\NewFunctionEntry{LFCalDispRectIntrinsics}{}
{Helper for setting up and visualizing intrinsics requested in rectification, see also \CiteFunction{LFRecenterIntrinsics}.}

\NewFunctionEntry{LFCalRectifyLF}{}
{Applies a calibration to rectify a light field. The desired intrinsic matrix can be provided, or computed automatically from the calibrated intrinsics. Demonstrated by \CiteFunction{LFUtilDecodeLytroFolder}.}

\NewFunctionEntry{LFRecenterIntrinsics}{}
{Recenters a light field intrinsic matrix, useful for modifying intrinsics requested in \CiteFunction{LFCalRectifyLF}, see \CiteFunction{LFCalDispRectIntrinsics}.}
...
{Recursively searches a folder for files matching one or more patterns.  Refer to this to understand the path parameters to \CiteFunction{LFUtilDecodeLytroFolder}, \CiteFunction{LFUtilExtractLFPThumbs} and  \CiteFunction{LFUtilUnpackLytroArchive}.}
...
{Sets up the Matlab path to include the LF Toolbox. This must be re-run every time Matlab restarts, so consider adding a line to \SymbolText{startup.m} as shown in Sect.~\ref{sect_Setup}.
}
