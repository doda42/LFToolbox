...
{Construct a 4D plane filter in the frequency domain.  This is useful for selecting objects at a single depth from a lightfield, and is similar in effect to refocus using, for example, the shift sum filter \CiteFunction{LFFiltShiftSum}.  Apply this filter with \CiteFunction{LFFilt4DFFT}.}
...
{Similar to \CiteFunction{LFDispVidCirc}, but visits every light field view in sequence following horizontal and vertical lawnmower patterns.}
...
{Uses \CiteFunction{LFDispTiles} to display six different tilings of the light field.}

\NewFunctionEntry{LFDispProjSubfigs}{\NewTag}
{Uses \CiteFunction{LFDispProj} to display six different projections of the light field.}
