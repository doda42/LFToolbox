    ...
    for( PoseIdx = 1:CalOptions.NPoses )
        ...
        for( TIdx = CalOptions.IJVecToOptOver )
            for( SIdx = CalOptions.IJVecToOptOver )
				...
				if( isfield(CalOptions,'OutlierRejectThresh') )
					% 					CurDist3D = min(CurDist3D, CalOptions.OutlierRejectThresh);
% 					VaildMean = mean(CurDist3D(CurDist3D <= CalOptions.OutlierRejectThresh));
					CurDist3D( CurDist3D > CalOptions.OutlierRejectThresh ) = CalOptions.OutlierRejectThresh;
				end

				
				if( ismember(PoseIdx, PosesToDraw) )
					VPlotPos{PoseIdx} = [VPlotPos{PoseIdx}, [STPlaneIntersect(1:2,:); CurDist3D]];
					VPlotST{PoseIdx} = [VPlotST{PoseIdx}, [CurCheckerObs_Idx(1:2,:); CurDist3D]];
				end

				if( PoseIdx < 6 )
					VPlotDir = [VPlotDir, [RayDir(1:2,:); CurDist3D]];
				end
	...
	for( OutputFig = PosesToDraw )
		...
		grid on
		view(3)
		ddplot3(VPlotPos{OutputFig},'.');
		sfigure(1000+OutputFig);
		cla
		hold on
		grid on
		view(3)
		ddplot3(VPlotST{OutputFig},'.');
	end
